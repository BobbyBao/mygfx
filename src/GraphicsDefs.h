...
namespace mygfx {
	...
	inline constexpr TargetBufferFlags getTargetBufferFlagsAt(size_t index) noexcept {
		if (index == 0u) return TargetBufferFlags::COLOR_0;
		if (index == 1u) return TargetBufferFlags::COLOR_1;
		if (index == 2u) return TargetBufferFlags::COLOR_2;
		if (index == 3u) return TargetBufferFlags::COLOR_3;
		if (index == 4u) return TargetBufferFlags::COLOR_4;
		if (index == 5u) return TargetBufferFlags::COLOR_5;
		if (index == 6u) return TargetBufferFlags::COLOR_6;
		if (index == 7u) return TargetBufferFlags::COLOR_7;

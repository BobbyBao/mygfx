...
namespace mygfx
{
        ...
        {
            return (IsCompressed_ASTC_LDR(format) || IsCompressed_BC(format) || IsCompressed_ETC2_EAC(format) ||
                IsCompressed_PVRTC(format));
        }

        /*
        template<>
        bool fromString<VkFormat>(const String& strVal, VkFormat& val)
        {
#define CASE(fmt)\
    else if(strVal == #fmt)\
            val = VkFormat::VK_FORMAT_##fmt

            if (strVal == "UNDEFINED")
                val = VkFormat::VK_FORMAT_UNDEFINED;

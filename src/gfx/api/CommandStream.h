...
protected:
    ...
    constexpr explicit CommandBase(Execute execute) noexcept
        : mExecute(execute)
    {
    }
...
public:
    ...
    {
        return (v + (OBJECT_ALIGNMENT - 1)) & ~(OBJECT_ALIGNMENT - 1);
...
public:
    inline constexpr explicit NoopCommand(void* next) noexcept
        : CommandBase(execute)
        , mNext(intptr_t((char*)next - (char*)this))
    {
    }
};
...
class CommandStream {
    ...
    struct AutoExecute {
        T closure;
        inline explicit AutoExecute(T&& closure)
            : closure(std::forward<T>(closure))
        {
        }
...
#define DECL_DRIVER_API(methodName, paramsDecl, params)                     \
    ...
    {                                                                       \
        ...
        apply(&Driver::methodName, mDriver, std::forward_as_tuple(params)); \
    }
